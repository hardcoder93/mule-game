...
public class StoreMenu extends BuildingMenu implements Serializable{
	...
	private void initStoreMenu() {
		...
		errorMessage.setBounds(getX(), getY() + getHeight() - 30, getWidth(), 20);
		...
		for (int i = 0; i < 3; i++){
			...
			tmp.setBounds(getX() + getWidth()/2 + 20, getY() + 60 + 30*i, 144, 27);
			comboBoxes.add(tmp);
		}
	    ...
	    for (int i = 0; i < comboBoxLabels.length; i++){
	    	...
	    	comboBoxLabels[i].setBounds(getX() + getWidth()/2 + 170, getY() + 60 + 30*i, 100, 27);
	    ...
	    for (int i = 0; i < menuItems.length; i++){
	        menuItems[i] = new JTextArea();
	       	...
	       	if (i < comboBoxEntries[1].length){
		       	menuItems[i].setBounds(getX() + 40, getY() + 63 + i * 20, getWidth() / 2 - 5, 20); 
	       		menuItems[i].setTabSize(7);
	       	}else{
		       	menuItems[i].setBounds(getX() + 40, getY() + 78 + i * 20, getWidth() / 2 - 5, 20); 
